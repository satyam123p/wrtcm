...
function initializeLocalStream() {
    ...
    console.log('Peer 2 initialized local stream for demo2.webm');
}
...
function createPeerConnection(peerId) {
    ...
    peer.onicecandidate = ({ candidate }) => {
        if (candidate) {
            ...
            console.log(`Peer 2 sent ICE candidate to ${peerId}`);
        }
    };

    peer.ontrack = (event) => {
        console.log(`Peer 2 received video frames from ${peerId} (demo.webm)`);
        const remoteStream = event.streams[0];
        console.log(`Peer 2 stream from ${peerId} has ${remoteStream.getTracks().length} tracks`);
    };

    peer.onconnectionstatechange = () => {
        if (peer.connectionState === 'connected') {
            console.log(`Peer 2 WebRTC connection established with ${peerId}`);
    ...
    if (localStream) {
        localStream.getTracks().forEach(track => {
            ...
            console.log(`Peer 2 added ${track.kind} track to connection with ${peerId}`);
        });
    }
...
async function createOffer(peerId) {
    try {
        ...
        console.log(`Peer 2 sent offer to ${peerId}`);
    } catch (e) {
        console.error(`Peer 2 error creating offer for ${peerId}:`, e);
    }
}
...
socket.on('message', async (msg) => {
    ...
    try {
        ...
        console.error('Peer 2 error parsing message:', e);
        return;
    }
    ...
    try {
        if (data.type === 'init') {
            ...
            console.log(`Peer 2 sent answer to ${data.from}`);
            ...
            if (peer) {
                ...
                console.log(`Peer 2: Peer ${data.peerId} disconnected`);
            }
        }
    } catch (e) {
        console.error(`Peer 2 error handling message from ${data.from || 'unknown'}:`, e);
    }
});

socket.on('open', () => console.log('Peer 2 connected to signaling server'));
socket.on('error', (err) => console.error('Peer 2 WebSocket error:', err));
socket.on('close', () => {
    console.log('Peer 2 disconnected from signaling server');
