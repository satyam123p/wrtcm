...
async function sendVideoFrames(videoPath, peerConnection) {
    try {
        ...
        videoStream.on('data', (chunk) => {
            console.log(`Peer 2 streaming chunk from ${videoPath}: ${chunk.length} bytes`);
            // Placeholder for real frame injection
        });

        videoStream.on('end', () => {
            console.log(`Peer 2 finished streaming ${videoPath}`);
        });
    } catch (error) {
        console.error(`Peer 2 error streaming video from ${videoPath}:`, error);
    }
}
